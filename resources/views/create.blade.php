                ...
                <div class="row mt-3">
                    <div class="col-lg-4">
                        ...
                        <input class="form-control" type="text" id="value" name="value" placeholder="R$ 0,00"
                            value="{{ number_format($finance->value, 2, '.', ',') ?? '' }}" required>
                    </div>
                    <div class="col-lg-4">
                        <p class="margin-show">Selecione uma data:</p>
                        <input class="form-control" type="date" id="date" name="date"
                            value="{{ $finance->date ? $finance->date : $date->format('Y-m-d') }}">
                    </div>
                    <div class="col-lg-4">
                        ...
                        @foreach ($availableMoney as $availableMoneys)
                            <input name="available_money_id" id="available_money_id" value="{{ $availableMoneys->id }}"
                                hidden>
                            <span class="form-control">R$ {{ number_format($availableMoneys->to_spend, 2) ?? '' }}</span>
                        @endforeach
                    </div>
                </div>
                <div class="row mt-3">
